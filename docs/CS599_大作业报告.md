...
- **用户接口层**：CLI 入口（main.py）、Streamlit Web UI（app.py）、MCP 协议接口（tool_server.py）
- **编排层**：Orchestrator（LangGraph StateGraph），负责条件路由和状态管理
- **Agent 层**：4 个专业 Agent — Spec Architect、Developer Agent、Reviewer Agent、QA Agent
- **共享能力层**：File Tools、Shell Sandbox、ChromaDB、LangFuse
- **LLM 层**：DeepSeek API，所有 Agent 的推理基础

数据流向：用户接口层 → 编排层 → Agent 层 → 共享能力层 → LLM 层。
Agent 之间的协作由 Orchestrator 的状态机统一调度。

## 3.2 Agent 交互流程（LangGraph 状态图）

工作流状态机按以下顺序执行，各阶段之间由条件网关控制流转：

1. **Spec Architect** → 生成规格文档 → 网关检查规格质量
   - 不合格 → 返回 Spec Architect 重写（最多 3 次）
   - 合格 → 进入 Developer
2. **Developer Agent** → 根据规格生成代码 → 进入 Reviewer
3. **Reviewer Agent** → 审查代码质量 → 网关检查审查分数（≥75 通过）
   - 不合格 → 返回 Developer 修复（最多 3 次）
   - 合格 → 进入 QA Agent
4. **QA Agent** → 生成并执行测试 → 网关检查测试结果
   - 不通过 → 返回 Developer 修复（最多 3 次）
   - 通过 → 进入 Finalize → 结束
...
| 阶段 | 结果 |
|------|------|
| Spec 生成 | [OK] 生成 3 份完整规格文档 |
| 代码生成 | [OK] 生成 `calculator.py`（含异常处理 + 类型注解）|
| 代码审查 | [OK] 评分 85/100，通过审查 |
| 测试生成 | [OK] 生成 6 个测试用例，全部通过 |
...
| 加分项 | 状态 | 说明 |
|--------|:--:|------|
| MCP 协议 / Agentic RAG | [OK] | 完整 MCP Tool Server 实现 |
| 云服务器部署 | [OK] | Docker Compose 一键部署，可部署至任意云服务器 |
| 生产级水平 | [OK] | 错误处理、安全防护（路径穿越/命令注入）、LangFuse 可观测性 |
